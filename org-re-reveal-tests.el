      ...
      `((:string=
         ...
         ,(let ((sourcepath (expand-file-name (format "test-cases/test-%s.org" ,name)
            ...
            (with-temp-buffer
              ...
              (goto-char (point-min))
              (while (re-search-forward (rx "id=\"slide-org" (= 7 not-newline)) nil t)
                (replace-match "id=\"slide-org*******" nil nil))
              (goto-char (point-min))
              (while (re-search-forward (rx "#/slide-org" (= 7 not-newline)) nil t)
                (replace-match "#/slide-org*******" nil nil))
              (goto-char (point-min))
