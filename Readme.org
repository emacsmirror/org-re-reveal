...
#          https://gitlab.com/oer/org-re-reveal/commits/main
...
[[https://gitlab.com/oer/org-re-reveal/-/blob/main/Readme.org][the
...
| [[https://gitlab.com/oer/org-re-reveal][org-re-reveal]]                   | Fork of org-reveal, initially to add audio fragments, now with [[https://gitlab.com/oer/org-re-reveal/-/blob/main/CHANGELOG.org][various changes]] |
...
** Changelog
   See file [[https://gitlab.com/oer/org-re-reveal/tree/main/CHANGELOG.org][CHANGELOG]]
   ...
   /reveal.js/ 4.x or later versions, see [[#revealjs-version][below how to configure the version]].
...
** Local presentations and CORS security
   :PROPERTIES:
   :CUSTOM_ID: cors-settings
   :END:
   - Generated presentations can be opened and viewed in browsers
     (without the need for a web server)
     - Some features may be prohibited based on [[https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS/Errors/CORSRequestNotHttp][cross-origin request security settings]]
       - E.g., with audio, the browser console of Firefox may show:
         ...
         - [[https://kb.mozillazine.org/Security.fileuri.strict_origin_policy][Workaround]]
       - Maybe use option ~--allow-file-access-from-files~ with chromium
     - Maybe serve presentations with [[#local-web-server][local web server]]

** Local presentations with web server
   :PROPERTIES:
   :CUSTOM_ID: local-web-server
   :END:
   - As alternative to overriding [[#cors-settings][CORS settings]],
     start a web server for generated presentations on ~localhost~,
     e.g.:
     - MELPA package
       [[https://melpa.org/#/simple-httpd][simple-httpd]] with
       ~M-x httpd-serve-directory~ or
     - [[https://revealjs.com/installation/#full-setup][full setup of reveal.js]]
   - Suggestion
     - Embed resources (e.g., reveal.js, CSS, plugins) with relative
       paths into presentation (instead of absolute ~file~ paths)
     - Then, presentation works not only locally but also on public
       web server
...
** Install reveal.js
   ...
   *Note:* This is only necessary if you do not use [[https://gitlab.com/oer/emacs-reveal][emacs-reveal]],
   ...
   (the “Basic Setup” is sufficient to use org-re-reveal, but see
   [[#cors-settings]]).
   ...
   ([[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/test-remote-plugin.org][Sample file]])
...
*** Install org-re-reveal from GitLab
    ...
    ([[https://gitlab.com/oer/org-re-reveal/-/archive/main/org-re-reveal-main.zip][zip archive]]
...
** Licensing
   I suggest that you think about “good” licensing terms for the
   presentations that you create for and with org-re-reveal (the
   source files as well as generated ones).  I recommend that
   you publish them under free and open licenses as
   [[https://en.wikipedia.org/wiki/Open_educational_resources][Open Educational Resources (OER)]].
   This process is simplified with oer-reveal and emacs-reveal.
...
** Options and Plug-ins
   ...
   Options and keywords with their customizable variables are shown in
   the following table, with hints and more customizable variables
   afterwards.
...
| ~nil~                             | ~REVEAL_FOOTNOTES_DIV~                       | ~org-re-reveal-footnotes-div~            |
...
| ~nil~                             | ~REVEAL_KLIPSE_JS_INIT~                      | ~org-re-reveal-klipse-js-init~           |
...
#+REVEAL: split
   ...
   and to [[https://gitlab.com/oer/org-re-reveal/tree/main/test-cases][test-cases in the GitLab repository]].
...
#+REVEAL: split
   *IMPORTANT*: Absolute paths as illustrated above hinder transfer of
   files between machines and servers; in addition, they violate the
   [[#cors-settings][same-origin security policy]].  Use of relative
   paths (the default) is recommended.

   In general, a presentation may embed lots of resources (JavaScript,
   CSS, images), and Org mode distinguishes the *publication* of
   projects (with multiple files) from the *export* of an individual
   Org source file.  The publication process usually creates a
   self-contained directory with all necessary resources, including
   the exported presentation(s).  Such directories can then be served
   from any web server.

   See [[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/publish.el?ref_type=heads][publish.el]],
   which defines a publication process that exports this Readme and
   selected test cases (executed in that
   [[https://gitlab.com/oer/org-re-reveal/-/blob/main/.gitlab-ci.yml?ref_type=heads][CI/CD
   pipeline on GitLab]], using functionality of oer-reveal), with the
   result being served as
   [[https://oer.gitlab.io/org-re-reveal/][GitLab Pages]].
...
** Images
   ...
   ca. line 130 in
   [[https://gitlab.com/oer/oer-reveal/-/raw/main/org/config.org][this file]],
...
*** Customize the Title Slide
    ...
    [[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/test-title-slide.org][test-title-slide.org]]
    for an example including ~REVEAL_ACADEMIC_TITLE~,
    ~REVEAL_TALK_URL~, ~REVEAL_TALK_QR_CODE~
    and
    [[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/test-title-slide-notes.org][test-title-slide-notes.org]]
    for a title slide including speaker notes (obsolete since version
    3.25.0; see [[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/test-title-slide-notes-2.org][test-title-slide-notes-2.org]]
...
** Slide Numbering
   ...
   Supported format strings can be found in the [[https://revealjs.com/slide-numbers/][reveal.js manual]].
...
** Slide Containers
   ...
   See [[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/test-slide-container.org][this slide container test case]]
   and [[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/test-slide-grid-wrapper.org][that slide grid test case]].
...
*** External plugins
    ...
    directory of the
    [[https://github.com/rajgoel/reveal.js-plugins/tree/main/chalkboard][Chalkboard plugin]]
...
#+END_SRC
    See also
    [[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/test-plugin-css.org][this
    test file for CSS with plugins]]
...
#+REVEAL: split
    ...
    See also
    [[https://gitlab.com/oer/org-re-reveal/-/issues/101][issue #101]]
    for auto-animated code blocks.
...
** Editable and Executable Source Code
   ...
   [[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/test-klipsify.org][test file]]
   to get started.
...
** Footnotes
  ...
  :CUSTOM_ID: footnotes
  :END:
  - Use footnotes as usual
  - Two options
    1. Add a slide “Footnotes”, where all footnotes are collected (see [[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/test-footnotes.org][test-footnotes.org]])
    2. Use startup option ~fnlocal~ to create footnote directly on slide (see [[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/test-footnotes-local.org][test-footnotes-local.org]])
...
*** Use MathJax
    ...
    1. Reveal.js added the ~math~ plugin in version 4.  Add that to
       ~org-re-reveal-plugins~.
    2. If org-re-reveal detects LaTeX math code and the ~math~ plugin
       is not used, a ~script~ element loads the MathJax library.  Set
       option =REVEAL_MATHJAX_URL= to a remote or local MathJax
       location (if you are not happy with the default value of
       ~org-re-reveal-mathjax-url~).

*** Configuration of MathJax
    The [[https://revealjs.com/math/][Reveal.js documentation]]
...
*** Text-To-Speech (TTS) for Speaker Notes
    Version 3.19.0 of org-re-reveal introduces support for TTS, which
    is complemented by functionality in emacs-reveal.

    Briefly, org-re-reveal can export notes from presentations to text
    files, from which audio files can be generated (e.g., with
    emacs-reveal).  The
    [[https://github.com/rajgoel/reveal.js-plugins/tree/main/audio-slideshow][audio-slideshow plugin]]
    can then play generated audio.  The result can serve as video mode for
    presentations, without the hassle of audio recording/cutting (and
    with the usual usability advantages of reveal.js presentations
    over videos).

    See variable ~org-re-reveal-with-tts~, whose doc string points to
    [[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/test-notes-for-tts.org][this sample presentation]]
    and this [[https://oer.gitlab.io/emacs-reveal-howto/tts-howto.html][TTS howto]]
    for emacs-reveal, both of which document various aspects of the
    TTS process.

    If you do not want to use emacs-reveal (or its Docker images),
    [[https://gitlab.com/oer/org-re-reveal/-/issues/92#note_1710604946][the script in that comment]]
    may help you getting started.

** Multiplexing
   Reveal.js supports [[https://github.com/reveal/multiplex][multiplexing]],
   which allows your audience to view
   the slides of the presentation you are controlling on their own phone, tablet
   or laptop.  As the main presentation navigates the slides, all client
   presentations will update in real time.

   Please see
   [[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/test-multiplex.org][this sample presentation for instructions]].
   ...
   ([[https://gitlab.com/oer/org-re-reveal/-/issues/98][see this issue]]).
...
** Table of Contents at Specified Position
   ...
   [[https://gitlab.com/oer/org-re-reveal/-/blob/main/test-cases/test-slide-numbers-reveal-toc.org][test file]]):
...
[[https://gitlab.com/oer/emacs-reveal-howto/-/blob/main/howto.org][howto]]
for an example.
