...
#+REVEAL_THEME: moon
#+REVEAL_HLEVEL: 2
#+REVEAL_HEAD_PREAMBLE: <meta name="description" content="Org-revealjs Introduction.">
#+REVEAL_POSTAMBLE: <p> Based upon work by yjwen. </p>
#+REVEAL_PLUGINS: (markdown notes)
#+REVEAL_EXTRA_CSS: ./local.css

[[https://www.gnu.org/licenses/gpl-3.0.html][https://img.shields.io/:license-gpl3-blue.svg]]

* Note

The project org-revealjs started as fork of
[[https://github.com/lechten/org-reveal][org-reveal]].

If you are interested in org-revealjs, I suggest that you take
a look at [[https://gitlab.com/oer/emacs-reveal][emacs-reveal]], which
embeds org-revealjs, reveal.js, and useful plugins as submodules,
...
* Reveal.js and org-revealjs
  ...
  - *Org-revealjs* exports your [[https://orgmode.org/][Org]]
    documents to reveal.js presentations.\\
    With org-revealjs, you can create beautiful presentations with 3D
...
* Requirements and Installation
  ...
  - And, of course, GNU Emacs, version 24.4 or later.
...
** Install reveal.js
   ...
   see the section [[https://gitlab.com/oer/org-revealjs#set-the-location-of-revealjs][Set the location of reveal.js]]

** Install org-revealjs from MELPA

   Org-revealjs is provided as MELPA package.  Use of MELPA requires
   some setup code in your ~.emacs~ file.
   See [[https://melpa.org/#/getting-started][here to get started with MELPA]].

** Install org-revealjs from GitLab

   You can download org-revealjs as archive from GitLab
   ([[https://gitlab.com/oer/org-revealjs/-/archive/master/org-revealjs-master.zip][zip archive]]
   and several ~tar~ variants; note the download icon on the
   [[https://gitlab.com/oer/org-revealjs][project page]]).

   Alternatively, you can clone the
   [[https://gitlab.com/oer/org-revealjs][repository of org-revealjs]]:
   #+BEGIN_SRC sh
   git clone https://gitlab.com/oer/org-revealjs.git
   #+END_SRC

   Add the directory of =org-revealjs.el= to your Emacs's ~load-path~, and add the
   ...
   *Note*: It is suggested to use a recent version of Org mode with
   org-revealjs.  [[https://orgmode.org/][Installation of Org mode.]]

* Configuration

** Set the location of reveal.js

   Org-revealjs must know where reveal.js is on your computer before
   ...
   Changing =org-revealjs-root= 's value will change the location
...
#+BEGIN_SRC lisp
(setq org-revealjs-root "file:///d:/reveal.js")
#+END_SRC
...
#+REVEAL: split
   *IMPORTANT*: If you use absolute paths to reveal.js as illustrated
   ...
   a CDN instead of downloading a local copy.  The following URL
   refers to an old version of reveal.js, though.
...
*** Url form for file location
    ...
    "file:///Users/username/org-revealjs/Readme.org".  This file in
    Ubuntu would be "file:///home/username/org-revealjs/Readme.org" and
    in Windows this file would be
    "file:///c:/Users/username/org-revealjs/Readme.org".  For more
    detail on this standard please refer to
    [[https://en.wikipedia.org/wiki/File_URI_scheme]]

** First Try

   The library org-revealjs needs to be loaded first.  That can happen
   either by adding ~(require 'org-revealjs)~ to your ~.emacs~ file or
   manually: Type "M-x load-library", then "org-revealjs".

   Now you can export this manual, ~Readme.org~, into a reveal.js
   presentation by typing "C-c C-e R R".
...
** The HLevel
   ...
   Org-revealjs maps each heading and its contents to one reveal.js
   slide. Since reveal.js arranges slides into a 2-dimensional matrix,
   org-revealjs use a *HLevel* value to decide whether to map headings to horizontal
   or vertical slides.
...
*** Configure HLevel's Value
    ...
    * Change variable =org-revealjs-hlevel='s value to set HLevel globally.\\
      For example, add the following statement to your =.emacs= file.
#+BEGIN_SRC lisp
(setq org-revealjs-hlevel 2)
#+END_SRC
...
** Set The Title Slide
   By default, org-revealjs generates a title slide displaying the
   ...
   ~org-revealjs-title-slide~ to ~nil~, or add ~reveal_title_slide:nil~ to
   ~#+OPTIONS:~ line.

   To restore the default title slide, please set variable
   ~org-revealjs-title-slide~ to ~'auto~.

*** Customize the Title Slide

    To customize the title slide, please set ~org-revealjs-title-slide~
...
#+REVEAL: split
    ...
    set ~org-revealjs-title-slide~ to the name of that file.
...
#+ATTR_REVEAL: :frag t
    ...
    * appear
    ...
    global variable ~org-revealjs-default-frag-style~.
...
*** List Fragments with Audio
    ...
    You can also use org-revealjs in combination with the
...
#+REVEAL: split
    - Custom variables:\\
      =org-revealjs-control=, =org-revealjs-progress=,
      =org-revealjs-history=, =org-revealjs-center=,
      =org-revealjs-rolling-links=, =org-revealjs-keyboard=,
      =org-revealjs-mousewheel=, =org-revealjs-defaulttiming=,
      =org-revealjs-fragmentinurl=, =org-revealjs-pdfseparatefragments=,
      =org-revealjs-overview=, =reveal_inter_presentation_links=
...
** Third-Party Plugins
   ...
   be loaded with org-revealjs. The paths to javascript loading code
   need to be customized in the variable
   ~org-revealjs-external-plugins~. This variable can be an
   ...
   have the form: ~" {src: \"%srelative/path/toplugin/from/reveal/root.js\"}"~

   If you need the async or callback parameters, include those too.

#+REVEAL: split

   If ~org-revealjs-external-plugins~ is a filename, that file must
...
=#+REVEAL_EXTERNAL_PLUGINS: external_plugins.js=
   ...
   In any case, org-revealjs will add the plugins to the dependencies
...
*** Using highlight.js
    ...
    ~#+REVEAL_HIGHLIGHT_CSS~ or the variable ~org-revealjs-highlight-css~.
...
** Editable Source Code
   ...
   order to edit code during a presentation.  At present, this
   capacity is turned on or off at time export using these defcustoms:
   - ~org-revealjs-klipsify-src~
   - ~org-revealjs-klipse-css~
   - ~org-revealjs-klipse-js~
   This feature is turned off by default and needs to be switched on
   with ~org-revealjs-klipsify-src~.  At present code editing is
   supported in javacript, clojure, php, ruby, scheme, and python
   only.
...
** Preamble and Postamble
   ...
   =org-revealjs-preamble= and =org-revealjs-postamble=.
   ...
   variable =org-revealjs-head-preamble= or option
...
** Speaker Notes
   ...
   Org-revealjs recognize texts between =#+BEGIN_NOTES= and =#+END_NOTES=
...
#+REVEAL: split
   ...
   variable =org-revealjs-plugins=, please make sure ~notes~ is in the
...
*** Easy-Template for Speaker Notes
    ...
    Org-revealjs registers 'n' as the key for speaker notes easy-template.
    So you can press '<' followed by 'n' and then press TAB, the ~#+BEGIN_NOTES~
    and ~#+END_NOTES~ pair is inserted automatically.

    Customize ~org-revealjs-note-key-char~ to change the default key
...
** Multiplexing
   ...
   *Warning!* The following is untested with ~org-revealjs~!
...
** Select Built-In Scripts
   ...
   Set option =REVEAL_PLUGINS= or variable =org-revealjs-plugins= to a
...
#+REVEAL: split
   ...
   * CSS needs to be *self-contained*.  Neither ~@import~ rules nor
     ...
     data URIs).
   * If you use ~org-revealjs-title-slide~ with custom HTML code and
...
#+REVEAL: split
   ...
   to use broken internal links, which are used in org-revealjs by
   default.  (Those links work with reveal.js, but are not understood
   by search engines.)  If you change ~org-revealjs--href-fragment-prefix~
   from its default to the value of ~org-revealjs--slide-id-prefix~,
...
** Custom JS
   ...
   custom variable ~org-revealjs-init-script~.

** Executable Source Blocks
   To allow live execution of code in some languages, enable the
   klipse plugin by setting ~org-revealjs-klipsify-src~ to non-nil.
   Src blocks with the languages contained in
   ~org-revealjs-klipse-languages~ will be executed with output shown
   in a console-like environment.  See the source code of
   ~org-revealjs-src-block~ for more details.
