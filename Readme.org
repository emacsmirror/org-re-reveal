...
#+REVEAL: split data-background="#555555"
   ...
   *Warning!* The logic to re-use the same attributes (e.g., background
   information) on the new slide is broken.  As a workaround, you can
...
#+END_SRC
   ...
   ~org-re-reveal-generate-custom-ids~ for more details.  In
   particular, the doc string explains the value ~draft~, which may be
   relevant when inserting a table of contents at a specific position
   as explained on the subsequent slide.
...
#+END_SRC
   ...
   When using the previous command and working on a document, value
   =draft= for variable =org-re-reveal-generate-custom-ids= may be of
   interest.
